...
LUA_CLIB = websocketnetpack clientwebsocket skynet socketdriver bson mongo md5 netpack \
  clientsocket memory profile multicast \
  cluster crypt sharedata stm sproto lpeg \
  mysqlaux debugchannel ltime crab utf8 $(TLS_MODULE)
...
$(LUA_CLIB_PATH)/clientsocket.so : lualib-src/lua-clientsocket.c lualib-src/lua-crypt.c lualib-src/lsha1.c | $(LUA_CLIB_PATH)
	$(CC) $(CFLAGS) $(SHARED) $^ -o $@ -lpthread
	
$(LUA_CLIB_PATH)/ltime.so : lualib-src/lua-ltime.c | $(LUA_CLIB_PATH)
	$(CC) $(CFLAGS) $(SHARED) $^ -o $@ -lpthread  
...
$(LUA_CLIB_PATH)/crab.so : 3rd/crab/lua-crab.c | $(LUA_CLIB_PATH)
	$(CC) $(CFLAGS) $(SHARED) -I3rd/crab $^ -o $@ 

$(LUA_CLIB_PATH)/utf8.so : 3rd/crab/lua-utf8.c | $(LUA_CLIB_PATH)
	$(CC) $(CFLAGS) $(SHARED) -I3rd/crab $^ -o $@
